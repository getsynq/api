...
- **`dbt_cloud.py`** — the full management lifecycle on a *managed* type:
  create → get → list → update (with `etag` optimistic concurrency) →
  disable → enable → health → delete.
- **`bigquery.py`** — the extra features *warehouse* types add: server-generated
...
Both scripts create a real integration, exercise the API, and delete it again at
