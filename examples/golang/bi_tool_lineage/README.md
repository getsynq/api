# Modelling a BI tool end to end
...
Metabase-specific: rename the structs in `metabase.go` and the mapping is
unchanged.

There is a Python counterpart under `python/bi_tool_lineage`. The two write the
same estate and print the same thing.
...
The program is idempotent — running it twice produces the same estate — so it is
a cron job, not a migration. It ends by reading the lineage back, which is worth
...
| File | What is in it |
|---|---|
| `metabase.go` | The BI tool's own metadata, and the sample content. Read this first |
| `main.go` | Auth, and the order the steps run in — three of them depend on an earlier one having landed |
| `sync.go` | The mapping: one function per concern, each explaining the choice it makes |
| `verify.go` | Reading the graph back |
...
the [`omni_types`](../omni_types) example for that half.
